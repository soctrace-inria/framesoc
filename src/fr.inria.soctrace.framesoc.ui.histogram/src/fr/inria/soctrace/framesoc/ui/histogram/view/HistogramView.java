...
public class HistogramView extends FramesocPart {
	...
	private Action createGanttAction() {
		...
		Action showGantt = new Action("Show in Gantt Chart", img) {
			...
			public void run() {
				...
				des.setStartTimestamp(((Double)plot.getDomainAxis().getRange().getLowerBound()).longValue());
		...
		showGantt.setEnabled(false);
		return showGantt;
	}

	private Action createTableAction() {
		ImageDescriptor img = ResourceManager.getPluginImageDescriptor(Activator.PLUGIN_ID, "icons/table.png");
	 ...
	 * If the need to differentiate among actions arises, 
	 * we can use actions IDs.
	 */
			...
			protected IStatus run(IProgressMonitor monitor) {
				...
				try {	
					...
					final JFreeChart chart = ChartFactory.createHistogram(HISTOGRAM_TITLE, X_LABEL, Y_LABEL, 
							dataset, PlotOrientation.VERTICAL, HAS_LEGEND, HAS_TOOLTIPS, HAS_URLS);
