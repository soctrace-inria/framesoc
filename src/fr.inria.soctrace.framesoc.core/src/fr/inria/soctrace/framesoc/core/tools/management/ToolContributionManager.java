...
public class ToolContributionManager {
	...
	// FramesocTool launcher cache
	private static Map<String, FramesocTool> launcherCache;
	static {
		launcherCache = new HashMap<>();
		...
		for (IConfigurationElement e : config) {
			try {
				...
				if (o instanceof FramesocTool) {
					launcherCache.put(e.getAttribute(EP_TOOL_ID), (FramesocTool) o);
				}
	...
	/**
	 * Get a list containing all the new plugin tool, not already present in the passed list of old
	 * tools.
	 * 
	 * @param oldTools
	 *            old tools
	 * @param reverseIdManager
	 *            id manager for newly created tools
	 ...
	 * 
	 * @param reverseIdManager
	 *            an id manager for newly created tools
	 ...
	 * 
	 * WARNING: the tool is executed INSIDE the UI Thread, so if long operations have to be done,
	 * the launch method must create its own Job (for example using the predefined class
	 * {@link PluginToolJob} or a class extending it).
	 * 
	 * @param tool
	 *            the plugin tool
	 * @param input
	...
	public static FramesocTool getToolLauncher(Tool tool) {
		...
		try {
			for (IConfigurationElement e : config) {
				String extensionId = e.getAttribute(EP_TOOL_ID);
				if (extensionId.equals(tool.getExtensionId())) {
					final Object o = e.createExecutableExtension(EP_TOOL_CLASS);
					if (o instanceof FramesocTool) {
