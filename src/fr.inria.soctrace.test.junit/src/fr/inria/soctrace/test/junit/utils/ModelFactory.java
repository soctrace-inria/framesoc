...
/**
 * Create Self Contained model elements that can be used to test write/read
 * operation on clean DBs.
 * 
 * All create methods create an element of the model with all its dependencies:
 * e.g. a group result and all the entities (events, event types) being grouped.
 * 
 * The user must save all the dependencies in order to correctly retrieve an
 * element of the model: e.g. if a group must be saved, the entities being
...
class ModelFactory implements IModelFactory {
	...
	public Event createEvent(int category) {
		...
		case EventCategory.PUNCTUAL_EVENT:
			return createPunctualEvent(
					eId,
					epId,
					createEventType(0, EventCategory.PUNCTUAL_EVENT,
							new IdManager()), createEventProducer());
		case EventCategory.LINK:
			return createLink(eId, epId,
					createEventType(0, EventCategory.LINK, new IdManager()),
					createEventProducer());
		case EventCategory.STATE:
			return createState(eId, epId,
					createEventType(0, EventCategory.STATE, new IdManager()),
					createEventProducer());
		case EventCategory.VARIABLE:
			return createVariable(
					eId,
					epId,
					createEventType(0, EventCategory.VARIABLE, new IdManager()),
					createEventProducer());
		}
		return null;
	}
	...
	public List<Event> createEvents(int n) {
		IdManager eId = new IdManager();
		IdManager epId = new IdManager();
		EventType et = createEventType(0, EventCategory.PUNCTUAL_EVENT,
				new IdManager());
	...
	public List<Event> createCategorizedEvents(int n) {
		...
		et = createEventType(etdm.getNextId(), EventCategory.PUNCTUAL_EVENT,
				eptIdm);
		for (int i = 0; i < n; ++i) {
	...
	public AnalysisResult createTraceResult(IdManager aid, String desc) {
		...
		processedTraceData.setProcessedTrace(ResourceLoader
				.getJunitProcessedTrace());
	...
	public AnalysisResult createSearchResult(IdManager aid, String desc) {
		...
		// RESULT DATA
		try {
			AnalysisResultSearchData searchData = new AnalysisResultSearchData(
					Event.class);
	...
	/**
	 * ROOT - TYPES 1. type1 2. type2 3. EMPTY - EVENTS 1. event1 2. event2 3.
	 * EMPTY
	 */
	@Override
	public AnalysisResult createGroupResult(IdManager aid, String desc) {
		...
		try {
			...
			UnorderedGroup root = new UnorderedGroup(idManager.getNextId(),
					null);
			root.setName("ROOT");

			OrderedGroup pattern = new OrderedGroup(idManager.getNextId(),
					EventType.class);
			pattern.setName("TYPES");
			pattern.addSon(et1, 0);
			pattern.addSon(et2, 1);
			UnorderedGroup empty = new UnorderedGroup(idManager.getNextId(),
					null);
			empty.setName("EMPTY");
			pattern.addSon(empty, 2);

			OrderedGroup example = new OrderedGroup(idManager.getNextId(),
					Event.class);
			...
			AnalysisResultGroupData groupData = new AnalysisResultGroupData(
					root);
	...
	public AnalysisResult createAnnotationResult(IdManager aid, String desc) {
		...
		AnnotationType annotationType = new AnnotationType(
				typeIdManager.getNextId());
		annotationType.setName("DECODING_MEMORY_USAGE");

		// ANNOTATION PARAM TYPE
		IdManager paramTypeIdManager = new IdManager();
		AnnotationParamType fx = new AnnotationParamType(
				paramTypeIdManager.getNextId());
		AnnotationParamType fy = new AnnotationParamType(
				paramTypeIdManager.getNextId());
		...
		Annotation firstCpuAnnotation = new Annotation(
				annotationIdManager.getNextId());
		Annotation secondCpuAnnotation = new Annotation(
				annotationIdManager.getNextId());
	 ...
	 * @return
	 */
	private Event createEvent(IdManager eId, IdManager epId, EventType type,
			EventProducer prod) {
	 ...
	 */
	private Link createLink(IdManager eId, IdManager epId, EventType type,
			EventProducer prod) {
	 ...
	 * @return
	 */
	private PunctualEvent createPunctualEvent(IdManager eId, IdManager epId,
			EventType type, EventProducer prod) {
	 ...
	 * @return
	 */
	private Variable createVariable(IdManager eId, IdManager epId,
			EventType type, EventProducer prod) {
	 ...
	 * @return
	 */
	private State createState(IdManager eId, IdManager epId, EventType type,
			EventProducer prod) {
