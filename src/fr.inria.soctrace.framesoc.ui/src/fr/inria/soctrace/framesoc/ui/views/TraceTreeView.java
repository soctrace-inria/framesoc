...
public class TraceTreeView extends ViewPart implements IFramesocBusListener {
	...
	/**
	 * TODO get the command from the extension point
	 * ID of the commands we have to remove for multiple selection
	 */
	...
	private ISelectionListener listener = new ISelectionListener() {
		public void selectionChanged(IWorkbenchPart sourcepart, ISelection selection) {
			if (sourcepart.equals(TraceTreeView.this) && selection instanceof IStructuredSelection) {
				...
				FramesocBus.getInstance().setVariable(FramesocBusVariable.TRACE_VIEW_CURRENT_TRACE_SELECTION, ss);
				logger.debug("Selected trace: {}", FramesocBus.getInstance().getVariable(FramesocBusVariable.TRACE_VIEW_SELECTED_TRACE).toString());
				logger.debug("Current trace selection: {}", FramesocBus.getInstance().getVariable(FramesocBusVariable.TRACE_VIEW_CURRENT_TRACE_SELECTION).toString());
		...
		viewer.addDoubleClickListener(new IDoubleClickListener() {
			...
			public void doubleClick(DoubleClickEvent event) {
				...
				if (!TraceSelection.isSelectionValid(thisSelection) || 
						!FramesocPartManager.getInstance().isFramesocPartExisting(FramesocViews.HISTOGRAM_VIEW_ID)) {
					return;
				}
	...
	private void buildToolbar(final Composite parent) {
		...
		expandAction.setImageDescriptor(ResourceManager.getPluginImageDescriptor(Activator.PLUGIN_ID, "icons/expandall.gif"));
		...
		collapseAction.setImageDescriptor(ResourceManager.getPluginImageDescriptor(Activator.PLUGIN_ID, "icons/collapseall.gif"));
	...
	public void handle(String topic, Object data) {
		if (topic.equals(FramesocBusTopic.TOPIC_UI_FOCUSED_TRACE) && data!=null) {
			...
			FramesocBus.getInstance().setVariable(FramesocBusVariable.TRACE_VIEW_SELECTED_TRACE, (Trace)data);
			FramesocBus.getInstance().setVariable(FramesocBusVariable.TRACE_VIEW_CURRENT_TRACE_SELECTION, sel);
			logger.debug("Selected trace: {}", FramesocBus.getInstance().getVariable(FramesocBusVariable.TRACE_VIEW_SELECTED_TRACE).toString());
			logger.debug("Current trace selection: {}", FramesocBus.getInstance().getVariable(FramesocBusVariable.TRACE_VIEW_CURRENT_TRACE_SELECTION).toString());
