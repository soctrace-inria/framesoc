...
/**
 * Singleton to access Framesoc parts management functionalities.
 * It is created at UI plugin startup (@see FramesocUiStartup).
 * 
 * <p>
...
public final class FramesocPartManager implements IFramesocBusListener {
	 ...
	 * 
	 * I have to set highlighting directly here and avoid sending the
	 * focused trace event on the Framesoc bus, in order to avoid 
	 * recursion: in fact, the trace tree view listens to such event too,
	 * so it would change its selection, and trigger this listener
	 * again....
	 */
	...
	private class ViewDesc {
		...
		public String toString() {
			return "ViewDesc [maxInstances=" + maxInstances + ", instances=" + instances + ", openParts=" + openParts + "]";
		}	
	}
	 ...
	 * 
	 * <p>
	 * If an empty view corresponding to this ID is present, it is
	 * used. Otherwise a new one is created and activated, if the 
	 * maximum number of instance has not been reached yet.
	 * 
	 * @param viewID view ID corresponding to an existing Framesoc analysis view
	 * @param trace the trace we want to load, or null if we need an empty view
	 * 
	 * @return a view, or null if the passed ID does not correspond
	 * to a Framesoc view, if the maximum number of instances for the
	 * view has been reached, if PartInitException is launched.
	 */
	...
	/**
	 * Close all the instances (except one) for the open
	 * Framesoc views.
	 */
				...
				for (int w = 0; w < windows.length; w++) {
					...
					for (int p = 0; p < pages.length; p++) {
						...
						for (int v = 0; v < viewRefs.length; v++) {
							...
							logger.debug("name: {}, sec id: {}", viewRef.getPartName(), viewRef.getSecondaryId());
							// count the FramesocPart instances
							ViewDesc desc = viewDescMap.get(viewRef.getId());
							if ( desc != null ) {
								logger.debug("found desc name: {}, sec id: {}", viewRef.getPartName(), viewRef.getSecondaryId());
								framesocRefs.add(viewRef);
								desc.instances++;
								desc.openParts.add((FramesocPart)viewRef.getPart(true));
							} else {
								logger.debug("not found desc name: {}, sec id: {}", viewRef.getPartName(), viewRef.getSecondaryId());
							}
						}
					}
				}
	...
	public void handle(FramesocBusTopic topic, Object data) {
		if (topic.equals(FramesocBusTopic.TOPIC_UI_HISTOGRAM_DISPLAY)) {
			...
			OpenFramesocPartStatus status = getPartInstance(FramesocViews.HISTOGRAM_VIEW_ID, des.getTrace());
			if (status.part == null) 
				return;
			status.part.showTrace(des.getTrace(), des);
			updateTitlesHighlight((Trace)des.getTrace());
		} 
		else if (topic.equals(FramesocBusTopic.TOPIC_UI_TABLE_DISPLAY_TIME_INTERVAL) && data!=null) {
			logger.debug("Topic table interval");
			TraceIntervalDescriptor des = (TraceIntervalDescriptor)data;
			OpenFramesocPartStatus status = getPartInstance(FramesocViews.EVENT_TABLE_VIEW_ID, des.getTrace());
			if (status.part == null) 
				return;
			status.part.showTrace(des.getTrace(), des);
			updateTitlesHighlight((Trace)des.getTrace());
		} else if (topic.equals(FramesocBusTopic.TOPIC_UI_GANTT_DISPLAY_TIME_INTERVAL) && data!=null) {
			logger.debug("Topic gantt interval");
			TraceIntervalDescriptor des = (TraceIntervalDescriptor)data;
			OpenFramesocPartStatus status = getPartInstance(FramesocViews.GANTT_CHART_VIEW_ID, des.getTrace());
			if (status.part == null) 
				return;
			status.part.showTrace(des.getTrace(), des);
			updateTitlesHighlight((Trace)des.getTrace());
	...
	/**
	 * Update the Framesoc parts view names, highlighting
	 * them if the selected trace is the one shown in the view,
	 * unhighlighting otherwise.
	 * 
	 * We use the asyncExec to avoid a non-deterministic buggy
	 * behavior: for the pie-chart view, the show view event
	 * does not highlight the name otherwise...
	 *  
	 * @param trace selected trace
	 */
	 ...
	 * 
	 * @param framesocPart the part to dispose
	 */
	...
	/**
	 * Create and activate a new FramesocPart, assigning
	 * a random secondary ID.
	 * 
	 * @param id primary ID of a FramesocPart
	...
	private FramesocPart createNewView(String id) {
		...
		try {
			...
			FramesocPart view = (FramesocPart)page.showView(id, secondaryId, IWorkbenchPage.VIEW_ACTIVATE);
			return view;		
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void loadDescMap() {
		int max = 5;
		try {
			max = Integer.valueOf(Configuration.getInstance().get(SoCTraceProperty.max_view_instances));
		} catch (NumberFormatException e) {
			logger.error(SoCTraceProperty.max_view_instances.toString() + " is not an integer, using " + max + " instead.");
		}
	...
	/**
	 * Look for a FramesocPart for the given id with 
	 * the given trace loaded inside.
	 * 
	...
	/**
	 * Look for a FramesocPart for the given id without a 
	 * loaded trace inside.
	 * 
	 * @param viewId view ID
	 * @return the part, or null if not found
